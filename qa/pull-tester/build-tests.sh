...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /home/ahmed/digitalcoin/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /home/ahmed/digitalcoin/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /home/ahmed/digitalcoin/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /home/ahmed/digitalcoin/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /home/ahmed/digitalcoin/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /home/ahmed/digitalcoin/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /home/ahmed/digitalcoin/linux-build
make check

# Run RPC integration test on Linux:
/home/ahmed/digitalcoin/qa/rpc-tests/wallet.sh /home/ahmed/digitalcoin/linux-build/src
/home/ahmed/digitalcoin/qa/rpc-tests/listtransactions.py --srcdir /home/ahmed/digitalcoin/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /home/ahmed/digitalcoin/win32-build
  make check
fi
